...
conditions = c('calcified', 'control')
celltype = c("VSMC_0", "VSMC_1", "VSMC_5")
...
#DE clusters
de.files <- list(response.0, response.1, response.2, response.3, response.4, response.5, response.6, response.7, response.8, response.9, response.10, response.11, response.12)
...
#All
all.markers <- list(osteo=osteo, ecm=ecm, phos=phos, inflam=inflam, others=others)


#Clusters - VSMC, Fibroblasts and Pericytes
clusters <- c(0, 1, 2, 4, 5, 9)

for (clust in clusters){
  # Get cell type
  cell.type <- label[names(label) == clust] %>% unlist() %>% str_replace("/", "_") %>% str_replace(" ", "_")
  cell.type <- paste(cell.type, paste0("cluster", clust), sep="_")
  outdir <- "curated_geneset_analysis/kidney"
  dir.create(outdir)
