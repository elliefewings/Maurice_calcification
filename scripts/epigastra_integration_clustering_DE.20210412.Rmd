...
#Doublet finding
doublets <- scDblFinder(sce = as.matrix(mh16@assays$RNA@counts), clusters = mh16@meta.data$seurat_clusters)
mh16$doublet_score <- doublets$scDblFinder.score
mh16$doublet <- doublets$scDblFinder.class

#Plot doublets
DimPlot(mh16, reduction = "umap", group.by = "seurat_clusters")
DimPlot(mh16, reduction = "umap", group.by = "doublet")

#Remove doublets
mh16 <- subset(mh16, subset= doublet == "singlet")
...
#Doublet finding
doublets <- scDblFinder(sce = as.matrix(mh15@assays$RNA@counts), clusters = mh15@meta.data$seurat_clusters)
mh15$doublet_score <- doublets$scDblFinder.score
mh15$doublet <- doublets$scDblFinder.class

#Plot doublets
DimPlot(mh15, reduction = "umap", group.by = "seurat_clusters")
DimPlot(mh15, reduction = "umap", group.by = "doublet")

#Remove doublets
mh15 <- subset(mh15, subset= doublet == "singlet")
...
features_calc <- c("PRG4", "SMAD3", "BCLAF1", "TCF21")
...
allfeatures <- c(features_VSMC,
                 ...
                 features_cellcycle,
                 features_immunecells,
                 features_calc) %>% unique()
```
...
#Checkpoint
#saveRDS(data, file = "integrateddata.epigastra.rds")

data <- readRDS("oldresults/integrateddata.epigastra.2.rds")
...
# Run UMAP
data <- RunUMAP(data, reduction = "pca", dims = 1:13, verbose = FALSE)

# Find neighbours
data <- FindNeighbors(data, reduction = "pca", dims = 1:13)
...
VlnPlot(data, feature = features_calc)
...
#Checkpoint 2
#saveRDS(data, file = "integrateddata_epigastra_p2.rds")
...
# Clean up
rm(avg, avg.0, avg.1, avg.2, avg.3, avg.4, avg.5, avg.6, avg.7, avg.8, avg.9, avg.10, response.bottom, response.top)


# Write csv
write.csv(response, "results/epigastra/epigastra.de.calcifiedcontrol.20210412.csv")

# Write csv per cluster
write.csv(response.0, "results/epigastra/de.cluster0_calcifiedcontrol.20210412.csv")
write.csv(response.1, "results/epigastra/de.cluster1_calcifiedcontrol.20210412.csv")
write.csv(response.2, "results/epigastra/de.cluster2_calcifiedcontrol.20210412.csv")
write.csv(response.3, "results/epigastra/de.cluster3_calcifiedcontrol.20210412.csv")
write.csv(response.4, "results/epigastra/de.cluster4_calcifiedcontrol.20210412.csv")
write.csv(response.5, "results/epigastra/de.cluster5_calcifiedcontrol.20210412.csv")
write.csv(response.6, "results/epigastra/de.cluster6_calcifiedcontrol.20210412.csv")
write.csv(response.7, "results/epigastra/de.cluster7_calcifiedcontrol.20210412.csv")
write.csv(response.8, "results/epigastra/de.cluster8_calcifiedcontrol.20210412.csv")
write.csv(response.9, "results/epigastra/de.cluster9_calcifiedcontrol.20210412.csv")
write.csv(response.10, "results/epigastra/de.cluster10_calcifiedcontrol.20210412.csv")
...
#Bind into one table
all.top <- rbind(top.0, top.1, top.2, top.3, top.4, top.5, top.6, top.7, top.8, top.9, top.10)
